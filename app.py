...
# ==============================================================================
# 1. ESTÉTICA (FONDO BLANCO, TEXTOS OSCUROS) - SIN CAMBIOS
# ==============================================================================
...
<div class="math-header">
    ...
    <div class="title-box">
        ...
        <p style="color: #444; font-family: 'Source Code Pro'; letter-spacing: 4px; font-size: 0.9rem;">
            UPRM MATHEMATICAL OPTIMIZATION ENGINE v13 (HÍBRIDO: AG + SA)
        </p>
    </div>
...
# ==============================================================================
# 4. CLASE PRINCIPAL HÍBRIDA (AG + SA)
# ==============================================================================
class HybridScheduler:
    def __init__(self, df_cursos, df_profes, df_salones, df_graduados, zona):
        ...
        # Parámetros de mutación (para SA)
        self.prob_mutar_profesor = 0.1
        self.prob_mutar_salon = 0.2
        self.mutacion_asistida = True  # siempre True

    # -------------------- Preasignación (igual que original) --------------------
        ...
        for s in secciones_multiple:
            ...
            if not asignado:
                ...
                if prof in capacidad_restante:
                    capacidad_restante[prof] -= creditos
        # Simulated annealing para balance
        def calc_penalidad():
            pen = 0
    ...
    # -------------------- Función de costo (idéntica a la original) --------------------
    ...
    # -------------------- ALGORITMO GENÉTICO (nuevo) --------------------
    def _crear_individuo_aleatorio(self):
        """Genera un individuo (solución) completamente aleatorio pero respetando restricciones locales."""
        sol = [None] * len(self.secciones)
        for i, s in enumerate(self.secciones):
            prof = getattr(s, 'prof_preasignado', 'TBA')
            sol[i] = self._crear_asignacion_temporal(s, prof=prof)
    ...
    def _cruce_uniforme(self, parent1, parent2):
        """Cruce uniforme: cada gen (asignación) se toma de uno de los padres con prob 0.5."""
        child1 = []
        child2 = []
        for g1, g2 in zip(parent1, parent2):
            if random.random() < 0.5:
                child1.append(deepcopy(g1))
                child2.append(deepcopy(g2))
            else:
                child1.append(deepcopy(g2))
                child2.append(deepcopy(g1))
        return child1, child2

    def _mutar_individuo_ag(self, individuo, pm=0.1):
        """Mutación asistida (igual que la mutación del SA)."""
        ...
        # Intentar cambiar a un profesor candidato diferente
        candidatos_prof = [c for c in s.cands if c in self.profesores and c != prof_actual]
        if candidatos_prof:
            nuevo_prof = random.choice(candidatos_prof)
        else:
            nuevo_prof = prof_actual
        # Elegir patrón y hora aleatorios
        ...
        salon = random.choice(salones_posibles) if salones_posibles else "TBA"
        nuevo[idx] = {'seccion': s, 'profesor': nuevo_prof, 'salon': salon, 'patron': patron, 'ini': ini}
        return nuevo

    def _seleccion_torneo(self, poblacion, fitnesses, k=3):
        """Selección por torneo (minimización: menor costo es mejor)."""
        mejor_idx = None
        mejor_costo = float('inf')
        for _ in range(k):
            idx = random.randint(0, len(poblacion)-1)
            if fitnesses[idx] < mejor_costo:
                mejor_costo = fitnesses[idx]
                mejor_idx = idx
        return deepcopy(poblacion[mejor_idx])

    def _ejecutar_ag(self, generaciones=100, tam_poblacion=50, pm=0.1, pc=0.8, status_text=None, bar=None):
        """Ejecuta el Algoritmo Genético y devuelve la mejor solución y su costo."""
        # Inicializar población
        poblacion = [self._crear_individuo_aleatorio() for _ in range(tam_poblacion)]
        fitnesses = [self._costo_total(ind) for ind in poblacion]
        mejor_global = deepcopy(poblacion[np.argmin(fitnesses)])
        mejor_costo_global = min(fitnesses)
        historial_ag = [mejor_costo_global]

        for gen in range(generaciones):
            nueva_poblacion = []
            # Elitismo: mantener el mejor individuo
            elite_idx = np.argmin(fitnesses)
            nueva_poblacion.append(deepcopy(poblacion[elite_idx]))
            # Generar el resto
            while len(nueva_poblacion) < tam_poblacion:
                # Selección de padres
                padre1 = self._seleccion_torneo(poblacion, fitnesses, k=3)
                padre2 = self._seleccion_torneo(poblacion, fitnesses, k=3)
                # Cruce
                if random.random() < pc:
                ...
                # Mutación
                hijo1 = self._mutar_individuo_ag(hijo1, pm)
                hijo2 = self._mutar_individuo_ag(hijo2, pm)
            ...
            if fitnesses[best_idx] < mejor_costo_global:
                mejor_costo_global = fitnesses[best_idx]
                mejor_global = deepcopy(poblacion[best_idx])
            historial_ag.append(mejor_costo_global)

            if status_text and (gen % 10 == 0 or gen == generaciones-1):
                status_text.markdown(f"**🧬 AG Generación {gen+1}/{generaciones}** | Mejor costo: {mejor_costo_global:.2f}")
            if bar:
                bar.progress((gen+1)/generaciones)

        return mejor_global, mejor_costo_global, historial_ag

    # -------------------- Simulated Annealing original (con pequeña adaptación) --------------------
    def _mutar_solucion_sa(self, sol):
        """Mutación para SA (igual que la original)."""
        nuevo = deepcopy(sol)
        idx = random.randint(0, len(nuevo)-1)
        asign = nuevo[idx]
        s = asign['seccion']
        ...
        for _ in range(15):
            ...
            if random.random() < self.prob_mutar_profesor:
            ...
            if random.random() < self.prob_mutar_salon:
    ...
    def _balancear_cargas(self, sol):
        """Balanceo de cargas (igual al original)."""
    ...
    def _resolver_conflictos_total(self, sol):
        """Reparación exhaustiva final (igual al original)."""
        def obtener_indices_conflictivos(sol):
            conflictos = []
            occ_prof = {}
            occ_salon = {}
        ...
        indices_conflictivos = obtener_indices_conflictivos(sol)
        ...
        for _ in range(max_attempts):
            ...
            for idx in indices_conflictivos:
                ...
                if mejores_opciones:
                    ...
                    mejora = True
            if mejora:
                nuevos_conflictos = obtener_indices_conflictivos(sol)
    ...
    # -------------------- OPTIMIZACIÓN HÍBRIDA (AG + SA) --------------------
    def optimizar_hibrido(self, iteraciones_sa=8000, ag_generaciones=100, ag_poblacion=50, ag_pm=0.1, ag_pc=0.8,
                          bar=None, status_text=None):
        """
        Fase 1: Algoritmo Genético durante `ag_generaciones`.
        Fase 2: Simulated Annealing durante `iteraciones_sa`.
        """
        historial_costos = []

        # ---- FASE 1: AG ----
        if status_text:
            status_text.markdown("**🧬 FASE 1: ALGORITMO GENÉTICO**")
        mejor_sol_ag, mejor_costo_ag, historial_ag = self._ejecutar_ag(
            generaciones=ag_generaciones,
            tam_poblacion=ag_poblacion,
            pm=ag_pm,
            pc=ag_pc,
            status_text=status_text,
            bar=bar
        )
        historial_costos.extend(historial_ag)

        # Establecer la mejor solución del AG como punto de partida para SA
        self.solucion = mejor_sol_ag
        self.mejor_solucion = deepcopy(mejor_sol_ag)
        self.mejor_costo = mejor_costo_ag
        self.historial_costos = historial_costos[:]

        # ---- FASE 2: SA (exactamente igual que tu optimizar original) ----
        if status_text:
            status_text.markdown("**🔥 FASE 2: SIMULATED ANNEALING**")
        temp_inicial = 5000.0
        sin_mejora = 0
        for it in range(iteraciones_sa):
            vecino, costo_vecino = self._mutar_solucion_sa(self.solucion)
            ...
            if it % 10 == 0 or it == iteraciones_sa - 1:
                if status_text:
                    fitness_actual = 10000 / (10000 + self.mejor_costo)
                    duros = int(self.mejor_costo // 10000)
                    status_text.markdown(f"**🔥 SA Iteración {it+1}/{iteraciones_sa}** | Conflictos Duros: {duros} | Costo Total: {self.mejor_costo:.2f} | Fitness: {fitness_actual:.5f}")
                if bar:
                    # Actualizar barra considerando ambas fases: AG completó su % de generaciones, SA completa el resto
                    total_iter = ag_generaciones + iteraciones_sa
                    current = ag_generaciones + it + 1
                    bar.progress(current / total_iter)
...
# ==============================================================================
# 7. UI PRINCIPAL (adaptada)
# ==============================================================================
def main():
    with st.sidebar:
        st.markdown("### 📅 Configuración")
        zona = st.selectbox("🏛️ Zona", ["CENTRAL", "PERIFERICA"])
        iteraciones = st.slider("Iteraciones de SA (Fase 2)", 100, 10000, 8000, help="Más iteraciones aumentan la probabilidad de cero conflictos.")
        ag_generaciones = st.slider("Generaciones del AG (Fase 1)", 20, 200, 100, help="Cuántas generaciones del Algoritmo Genético al inicio.")
        ag_poblacion = st.slider("Tamaño de población AG", 20, 100, 50, help="Número de individuos en el AG.")
    ...
    with c3: st.markdown("""<div class="status-badge">RESTRICCIONES FUERTES ACTIVAS | MODO HÍBRIDO (AG + SA)</div>""", unsafe_allow_html=True)
    ...
    if not file:
        ...
        if st.button("🚀 INICIAR OPTIMIZACIÓN HÍBRIDA (AG + SA)"):
            try:
                with st.spinner("Fase 1: Algoritmo Genético explorando... Fase 2: Simulated Annealing refinando..."):
                    ...
                    bar = st.progress(0)
                    status = st.empty()
                    mejor_sol, conflictos, historial = scheduler.optimizar_hibrido(
                        iteraciones_sa=iteraciones,
                        ag_generaciones=ag_generaciones,
                        ag_poblacion=ag_poblacion,
                        ag_pm=0.1,
                        ag_pc=0.8,
                        bar=bar,
                        status_text=status
                    ...
                    st.session_state.detailed_conflicts = scheduler._obtener_conflictos(mejor_sol) if hasattr(scheduler, '_obtener_conflictos') else []
    ...
    if 'master' in st.session_state:
        ...
        with t4:
            ...
            ax1.axvline(x=ag_generaciones, color='#8E6E13', linestyle='--', label='Fin del AG / Inicio SA')
            ax1.set_title("Crecimiento de Fitness (AG + SA)", color='black', pad=15)
            ax1.set_xlabel("Iteraciones totales", color='black')
