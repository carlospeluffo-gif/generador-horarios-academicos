...
class Profesor:
    def __init__(self, nombre, carga_min, carga_max, pref_dias, pref_horas,
        ...
        # Cargas
        try:
            self.carga_min = float(carga_min) if carga_min and carga_min != '' else 0.0
        except:
            self.carga_min = 0.0
        try:
            self.carga_max = float(carga_max) if carga_max and carga_max != '' else 12.0
        except:
            self.carga_max = 12.0
            
        self.pref_dias = str(pref_dias).strip() if pref_dias else ''
        self.pref_horas = str(pref_horas).strip() if pref_horas else 'ANY'
        
        self.preferencias = []
        if isinstance(preferencias_cursos, list):
            self.preferencias = [c.upper().strip() for c in preferencias_cursos if c and str(c).upper().strip() != 'NAN']
        
        # Compensación: robusta (1 o SI/SÍ/YES)
        try:
            comp_val = int(compensacion)
            self.compensacion = (comp_val == 1)
        except:
            self.compensacion = str(compensacion).upper().strip() in ('SI', 'SÍ', 'YES', '1')
        
        # Acepta grandes: similar a cursos_intensivos
        try:
            self.acepta_grandes = int(acepta_grandes)
        except:
        ...
        if bloqueo_dias:
            ...
            dias_limpios = str(bloqueo_dias).upper().replace(' ', '').replace(',', '')
            dias_set = set()
            i = 0
            while i < len(dias_limpios):
                if i+1 < len(dias_limpios) and dias_limpios[i:i+2] == 'MI':
                    dias_set.add('Mi')
                    i += 2
                else:
            ...
            if dias_set:
                try:
                    start_str = str(bloqueo_ini).strip() if bloqueo_ini else ''
                    end_str = str(bloqueo_fin).strip() if bloqueo_fin else ''
    ...
    if not file:
        st.markdown("""
            <div class='glass-card' style='text-align: center;'>
                ...
                <p>Asegúrese de que el archivo contiene las hojas: 'Cursos', 'Profesores', 'Salones'.<br>
                Las columnas de Profesores deben incluir: CURSOS_INTENSIVOS, ACEPTA_GRANDES, BLOQUEO_DIAS, BLOQUEO_HORA_INI, BLOQUEO_HORA_FIN, PREF1, PREF2, PREF3, COMPENSACION.</p>
            </div>
        """, unsafe_allow_html=True)
    else:
        if st.button("🚀 INICIAR OPTIMIZACIÓN ABSOLUTA"):
            with st.spinner("Balanceando cargas, consolidando secciones y resolviendo..."):
                ...
                # Reemplazar NaN por cadena vacía para evitar errores
                df_cursos = df_cursos.fillna('')
                df_profes = df_profes.fillna('')
                df_salones = df_salones.fillna('')
