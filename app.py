...
# ------------------------------------------------------------------------------
st.set_page_config(page_title="UPRM Scheduler Ultimate", page_icon="🏛️", layout="wide")
st.markdown("""
<style>
    ...
    .rum-header {
        ...
        padding: 25px 50px; background: linear-gradient(105deg, rgba(255,255,255,0.95) 0%, rgba(248,250,248,0.98) 100%);
        border-bottom: 6px solid #004B23; margin-bottom: 35px; border-radius: 0 0 30px 30px;
        box-shadow: 0 15px 30px -10px rgba(0,75,35,0.15); backdrop-filter: blur(5px); z-index: 10;
    }
    .rum-header::after {
    ...
    .title-box h1 {
        font-family: 'Playfair Display', serif; background: linear-gradient(135deg, #004B23 0%, #0A6B3A 80%);
        -webkit-background-clip: text; background-clip: text; color: transparent;
        font-size: 3.2rem; margin: 5px 0; letter-spacing: 3px; font-weight: 900;
    }
    ...
    .glass-card {
        background: rgba(255,255,255,0.7); backdrop-filter: blur(12px); border-radius: 24px; padding: 28px;
        border: 1px solid rgba(0,75,35,0.15); box-shadow: 0 20px 35px -8px rgba(0,75,35,0.1);
        margin-bottom: 25px; color: #1a1a1a;
    }
    .stButton > button {
        background: linear-gradient(145deg, #004B23 0%, #0A6B3A 100%); color: white; font-weight: 600;
        border-radius: 50px; width: 100%; border: none; height: 58px; font-size: 1.2rem;
        box-shadow: 0 8px 15px rgba(0,75,35,0.25); text-transform: uppercase;
    }
    .stDownloadButton > button {
        background: linear-gradient(145deg, #C69214 0%, #E6B422 100%); color: #1a1a1a; font-weight: 700;
        border-radius: 50px; box-shadow: 0 8px 15px rgba(198,146,20,0.25); height: 50px;
    }
    ...
    .status-badge {
        background: rgba(0,75,35,0.08); border: 1.5px solid #004B23; color: #004B23;
        padding: 14px 18px; border-radius: 60px; text-align: center; font-weight: 700;
    }
...
<div class="rum-header">
    ...
    <div class="title-box"><h1>UPRM TIMETABLE SYSTEM</h1><p><span class="subtitle-accent">COLEGIO DE ARTES Y CIENCIAS</span> · OPTIMIZACIÓN ULTIMATE</p></div>
...
# ------------------------------------------------------------------------------
# TABLAS Y UTILIDADES (SIN CAMBIOS)
# ------------------------------------------------------------------------------
COMPENSACION_TABLE = [
...
# ------------------------------------------------------------------------------
# NUEVO MOTOR ULTRA-HÍBRIDO (GARANTIZA CONFLICTO CERO)
# ------------------------------------------------------------------------------
class UltimateScheduler:
    def __init__(self, df_cursos, df_profes, df_salones, zona, df_grad=None, max_intentos=5):
        self.zona=zona
        # Salones
        ...
        # Precomputar opciones por sección
    ...
    def get_sec_creditos(self,s,prof):
        if prof in self.profesores and self.profesores[prof].compensacion: return get_creditos_reales(s.creditos,s.cupo)
        return float(s.creditos)

    def _preasignar_profesores_robusto(self):
        # (Misma lógica que antes, asigna profesores a secciones según candidatos y preferencias)
    ...
    def _evaluar_individuo(self, ind):
        """Retorna (conflictos_duros, costo_suave, carga_dict)"""
        duros=0; suave=0
        occ_prof=defaultdict(list); occ_sal=defaultdict(list)
        carga=defaultdict(float)
        for idx,op in enumerate(ind):
            if op is None or op>=len(self.opciones_por_seccion[idx]): duros+=10000; continue
            s=self.secciones[idx]; prof=s.prof_preasignado
            pat,h,sal=self.opciones_por_seccion[idx][op]
            if prof=="TBA" or sal=="TBA": duros+=10000; continue
            # Restricciones profesor
            if prof in self.profesores:
                po=self.profesores[prof]
                if po.acepta_grandes==0 and s.es_grande: duros+=10000
                es_int=any(c>=3 for c in pat['days'].values())
                if po.cursos_intensivos==0 and es_int: duros+=10000
                    ...
                    if puede_int: duros+=10000
                for (dset,st,en) in po.bloqueos:
                    for d in pat['days']:
                        if d in dset:
                            fin=h+int(pat['days'][d]*50)
                            if max(h,st)<min(fin,en): duros+=10000
                # Soft
            ...
            for d,c in pat['days'].items():
                ...
                if prof!="GRADUADOS":
                    for (ini_ex,fin_ex) in occ_prof[(prof,d)]:
                        if max(h,ini_ex)<min(fin,fin_ex): duros+=10000
        ...
        for p,c in carga.items():
            if p in self.profesores:
                ...
                if c>po.carga_max+1.5: duros+=10000
                if c<po.carga_min-1.5: duros+=10000
        # Doble rol graduados
                    ...
                    for d_d,c_d in p_d['days'].items():
                        ...
                        for d_r,c_r in p_r['days'].items():
                            if d_d==d_r:
                                ...
                                if max(ini_d,ini_r)<min(fin_d,fin_r): duros+=10000
        return duros,suave,dict(carga)

    def fitness(self, ind):
        d,s,_=self._evaluar_individuo(ind)
        if d>0: return d
    ...
    def _reparador_agresivo(self, ind):
        for _ in range(5):
            for idx in range(len(ind)):
                d,_,_=self._evaluar_individuo(ind)
                if d==0: break
    ...
    def _busqueda_tabu(self, ind, iters=800):
    ...
    def _simulated_annealing(self, ind, temp=1000, cooling=0.995, iters=2000):
        ...
        for _ in range(iters):
            ...
            vec=list(actual); vec[idx]=op
            d,_,_=self._evaluar_individuo(vec)
            if d>0: continue
    ...
    def _algoritmo_genetico(self, tam_pop=200, gens=300):
        # Inicialización
        pop=[]; fits=[]
        for _ in range(tam_pop):
            ...
            ind=self._reparador_agresivo(ind)
            pop.append(ind); fits.append(self.fitness(ind))
        ...
        for gen in range(gens):
            ...
            while len(nueva)<tam_pop:
                ...
                hijo=self._reparador_agresivo(hijo)
                nueva.append(hijo)
            pop=nueva; fits=[self.fitness(ind) for ind in pop]
            gen_best_idx=np.argmin(fits)
            if fits[gen_best_idx]<best_fit:
                best_fit=fits[gen_best_idx]; best=pop[gen_best_idx]
            if best_fit<10000: break
        return best

    def optimizar(self, bar=None, status=None):
        mejor_ind_global=None; mejor_fit_global=float('inf')
        historial=[]
        for intento in range(self.max_intentos):
            if status: status.markdown(f"**🔄 Intento {intento+1}/{self.max_intentos}** - Fase GA")
            ind=self._algoritmo_genetico(tam_pop=200, gens=300)
            d,_,_=self._evaluar_individuo(ind)
            if d>0:
                if status: status.markdown("**🔧 Fase Tabú** - Eliminando conflictos residuales")
                ind=self._busqueda_tabu(ind, iters=1000)
        ...
        # Fase SA para refinamiento
        if status: status.markdown("**❄️ Fase Recocido Simulado** - Optimizando preferencias")
        mejor_ind_global=self._simulated_annealing(mejor_ind_global, iters=2000)
        ...
        duros,_,_=self._evaluar_individuo(mejor_ind_global)
        conflictos=int(duros//10000) if duros>=10000 else 0
        return sol, conflictos, historial

    def _obtener_conflictos(self, sol):
        # Reconstruir individuo
        ind=[]
        ...
        duros,_,_=self._evaluar_individuo(ind)
        if duros==0: return []
        # Generar lista de conflictos simplificada
...
def main():
    ...
    if not file:
        ...
        if st.button("🚀 OPTIMIZAR (MODO ULTIMATE)"):
            with st.spinner("Procesando..."):
                ...
                sched=UltimateScheduler(df_c,df_p,df_s,zona,df_g,max_intentos=5)
                bar=st.progress(0); stat=st.empty()
                start=time.time()
                sol,conf,hist=sched.optimizar(bar,stat)
                bar.progress(1.0)
