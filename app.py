        ...
        if colegio_completo in programas_simplificados:
            for programa_key, programa_nombre in programas_simplificados[colegio_completo].items():
                credenciales[f"{usuario_simple}|{programa_key}"] = {
                    'usuario': usuario_simple, 'contraseña': programa_key,
                    'colegio_completo': colegio_completo, 'programa': programa_nombre,
                    'nivel': "Bachillerato"
                }
...
def mostrar_login_simplificado():
    ...
    with col2:
        ...
        with st.expander("ℹ️ ¿Cómo obtener mis credenciales?", expanded=False):
            ...
            **Usuarios disponibles (Ejemplos):**
            - `admin_empresas` / Contraseñas: `contabilidad`, `finanzas`
            - `artes_ciencias` / Contraseñas: `biologia`, `quimica`, `ingles`
            - `ingenieria` / Contraseñas: `ing_civil`, `ing_quimica`
            """)
...
class ConfiguracionSistema:
    """Clase que maneja la configuración del sistema, salones y la carga de datos del Excel."""
    ...
    def _obtener_horarios_prohibidos(self):
        ...
        if self.colegio_actual in ["COLEGIO DE ADMINISTRACIÓN DE EMPRESAS", "COLEGIO DE ARTES Y CIENCIAS"]:
            # Hora Universal RUM: Martes y Jueves (Ma, Ju) de 10:30 a 12:30
    ...
    def _cargar_datos_excel(self):
        ...
        for campo, col_default in mapeo_columnas.items():
            ...
            if encontrado:
                ...
                # Creación de columnas por defecto si no se encuentran
...
class AsignacionClase:
    """Clase que representa un cromosoma del GA, una asignación de clase (Día, Hora, Salón)."""
...
def evaluar_horario(asignaciones):
    ...
    Simula la penalización y bonificación de restricciones blandas.
    """
    ...
    # Se accede a la configuración global para obtener las restricciones
    global config 
    if 'config' in globals() and config and config.restricciones_globales:
            ...
            for dia, duracion in zip(asig.bloque["dias"], asig.bloque["horas"]):
                ...
                if dia in horarios_prohibidos:
                    for r_ini, r_fin in horarios_prohibidos[dia]:
                        ...
                        # Comprobar solapamiento con la Hora Universal
                        if not (fin_min <= r_ini_min or ini_min >= r_fin_min):
                            penalizacion_universal += 50
                            
    score -= penalizacion_universal
    
    # 2. Bonificación por agrupar clases del mismo curso (ejemplo simple de coherencia)
    cursos_agrupados = pd.DataFrame([{'Curso': a.curso, 'Profesor': a.profesor} for a in asignaciones])
    # Mayor desviación estándar en el conteo de cursos por (Curso, Profesor) puede indicar una mejor distribución/agrupación
    try:
        score += cursos_agrupados.groupby(['Curso', 'Profesor']).size().std() * 5 
    except:
        pass # Si solo hay una asignación o el cálculo falla
...
def buscar_mejor_horario(df_cursos_input, intentos=500):
    ...
    global config # Acceso a la configuración global
    if not config:
        st.error("Configuración del sistema no cargada.")
        return None, 0, None

    # Recopilar listas de elementos del GA
    profesores = df_cursos_input['Profesor'].unique().tolist()
    salones = config.salones
    ...
    for _, curso_info in df_cursos_input.iterrows():
        ...
        try:
            ...
            bloques_candidatos = [b for b in bloques_validos if b['creditos'] == curso_info['Créditos']]
            if not bloques_candidatos:
                # Si no hay bloque estándar, crear uno simple (ej. L:3)
                bloque = {"id": 99, "dias": ["Lu"], "horas": [curso_info['Créditos']], "creditos": curso_info['Créditos']}
            else:
                bloque = random.choice(bloques_candidatos)
            ...
            print(f"Error al generar asignación: {e}")
            continue 
...
def crear_tabla_horario_profesional(df_horario, filtro_tipo="completo", filtro_valor=None):
    ...
    periodos = []
    hora_inicio = a_minutos("07:30")
    hora_fin = a_minutos("19:30")    
    ...
    for _, fila in df_filtrado.iterrows():
        ...
        # Llenar solo el primer slot de 30 min para una clase que solapa un periodo
        clase_ya_asignada = False
        for periodo in periodos:
            if not (fin_clase <= periodo['inicio_min'] or inicio_clase >= periodo['fin_min']):
                if not clase_ya_asignada:
                    matriz_horario[periodo['periodo']][dia_completo] = info_curso
                    clase_ya_asignada = True
                else:
                    # Marcar slots subsiguientes como continuación
                    matriz_horario[periodo['periodo']][dia_completo] = "..."
    ...
    # Filtrar filas vacías (solo si todas las columnas de día están vacías)
    df_tabla['Clases_Dia'] = df_tabla[dias].apply(lambda row: any(row != ''), axis=1)
    df_tabla = df_tabla[df_tabla['Clases_Dia']].drop(columns=['Clases_Dia'])

    if df_tabla.empty:
        st.info("No hay clases programadas en el rango de horario seleccionado.")
        return
    
    # Aplicar estilos CSS para la tabla
    st.markdown("""
    <style>
    .horario-tabla {
...
def mostrar_portal_busqueda_secciones(df_horario):
    ...
    # Lógica de Filtrado (Aplicar las condiciones)
    df_filtrado = df_horario.copy()
    ...
    # Mostrar Resultados
    if num_resultados > 0:
        # Agrupar por sección para mostrar las franjas horarias consolidadas por curso/sección
        df_presentacion_agrupada = df_filtrado.groupby(['Curso', 'Sección', 'Profesor', 'Programa', 'Salón', 'Capacidad', 'Estudiantes']).agg(
            Horario=('Día', lambda x: ', '.join(f"{d} {h}" for d, h in zip(x, df_filtrado.loc[x.index, 'Hora Inicio'] + "-" + df_filtrado.loc[x.index, 'Hora Fin']))),
            Días=('Día', lambda x: ', '.join(x.unique()))
        ...
        df_final = df_presentacion_agrupada[['Curso', 'Sección', 'Profesor', 'Horario', 'Días', 'Salón/Recinto', 'Cap', 'Matrícula']]
...
def crear_calendario_interactivo(df_horario, filtro_tipo=None, filtro_valor=None, key="calendar"):
    """
    Función Placeholder para Calendario Plotly.
    Genera un gráfico vacío/simplificado para mantener la aplicación funcional.
    """
    df_temp = df_horario.copy()
    if filtro_tipo == "profesor" and filtro_valor and filtro_valor != "Todos los profesores":
        df_temp = df_temp[df_temp['Profesor'] == filtro_valor]
    
    # Creación de datos para Plotly (simplificado a puntos)
    df_temp['Hora_Inicio_Min'] = df_temp['Hora Inicio'].apply(a_minutos)
    df_temp['Día_Num'] = df_temp['Día'].map({'Lu': 1, 'Ma': 2, 'Mi': 3, 'Ju': 4, 'Vi': 5})
    
    fig = go.Figure()
    
    colores_cursos = generar_colores_cursos(df_temp)
    
    for curso, color in colores_cursos.items():
        df_curso = df_temp[df_temp['Curso'] == curso]
        if not df_curso.empty:
            fig.add_trace(go.Scatter(
                x=df_curso['Día_Num'],
                y=df_curso['Hora_Inicio_Min'],
                mode='markers',
                name=curso,
                marker=dict(size=10, color=color),
                hovertext=df_curso.apply(lambda r: f"Curso: {r['Curso']}-{r['Sección']}<br>Prof: {r['Profesor']}<br>Hora: {r['Hora Inicio']}-{r['Hora Fin']}<br>Salón: {r['Salón']}", axis=1),
                hoverinfo='text'
            ))
    
    fig.update_layout(
        title='Calendario Interactivo (Representación de Inicios de Clase)',
        xaxis=dict(tickmode='array', tickvals=[1, 2, 3, 4, 5], ticktext=['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes']),
        yaxis=dict(range=[a_minutos("19:30"), a_minutos("07:30")], title="Hora del Día"),
        height=600
    )
...
def mostrar_leyenda_cursos(colores_cursos, df_horario, filtro_tipo=None, filtro_valor=None):
    ...
    for curso, color in colores_cursos.items():
        legend_html += f"<div style='display: flex; align-items: center; margin-right: 15px; margin-bottom: 5px;'>"
        legend_html += f"<span style='width: 12px; height: 12px; background-color: {color}; border-radius: 3px; margin-right: 5px;'></span>"
        legend_html += f"<span>{curso}</span>"
        legend_html += "</div>"
    legend_html += "</div>"
...
def mostrar_tabs_horario_mejoradas(df_horario):
    ...
    with tab2:
        ...
        col1_filtro, col2_filtro = st.columns([2, 1])
        with col1_filtro:
            tipo_filtro = st.selectbox("🔍 Filtrar por:", ["Completo", "Profesor"], key="tipo_filtro_cal")
        with col2_filtro:
            filtro_valor = None
        ...
        with col1_info:
            st.info("💡 Tip: El calendario muestra los puntos de inicio de las clases. Para el horario detallado, use la **Tabla Profesional**.")
    ...
    with tab6:
        ...
        st.info("Las estadísticas completas (violaciones de Restricciones Duras, distribución horaria, etc.) se mostrarían aquí.")
...
def main():
    ...
    if not st.session_state.usuario_autenticado:
        ...
        with tab_info:
            ...
            st.markdown("- **Portal de Búsqueda:** Búsqueda flexible por curso y profesor.")
    else:
        ...
        global config # Necesario para que la variable global 'config' se actualice

        if uploaded_file is not None:
            # Crear un archivo temporal para que ConfiguracionSistema pueda leerlo
            ...
            # Limpiar el archivo temporal después de la configuración
            # os.remove(temp_file_path) # Comentar esto si necesitas debuguear
